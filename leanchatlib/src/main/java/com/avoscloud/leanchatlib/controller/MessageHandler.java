...
public class MessageHandler extends AVIMTypedMessageHandler<AVIMTypedMessage> {
  ...
  public void onMessage(AVIMTypedMessage message, AVIMConversation conversation, AVIMClient client) {
    ...
    if (ChatManager.getInstance().getSelfId() == null) {
      LogUtils.d("selfId is null, please call initWithUserId ");
      client.close(null);
    } else {
      if (!client.getClientId().equals(ChatManager.getInstance().getSelfId())) {
        ...
        if (!message.getFrom().equals(client.getClientId())) {//若消息不是来自自己
          if (NotificationUtils.isShowNotification(conversation.getConversationId())) {
            sendNotification(message, conversation);//发送通知
          }
          //增加未读消息
  ...
  private void sendNotification(AVIMTypedMessage message, AVIMConversation conversation) {
    if (null != conversation && null != message) {
      ...
      intent.putExtra(Constants.MEMBER_IMID, message.getFrom());
